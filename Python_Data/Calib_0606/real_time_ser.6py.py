...
with serial.Serial(sercom, 115200) as ser:  # check arduino port
    while(i<sizeall+10):
...
def update():
    ...
    global ave
    i = 0
    with serial.Serial(sercom, 115200) as ser:  
        while(i<(sizex-1)):
        # while(1):
            # ser.readline()
            line_dat = Sergetline(ser)
            print("line_dat", line_dat)
            datx[i] = line_dat
            i = i + 1
    ...
    # imu = datx[5, 4:6] # select no.5 imu data
